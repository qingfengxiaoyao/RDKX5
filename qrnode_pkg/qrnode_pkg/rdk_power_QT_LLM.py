...
from PyQt5.QtGui import QImage, QPixmap, QPainter, QPen, QFont, QTextCursor
import os
import requests
import json
import time
...
class AIClient:
    def __init__(self, api_url="http://localhost:11434/api/generate", model="deepseek-r1:1.5b"):
        self.api_url = api_url
        self.model = model
        self.headers = {'Content-Type': 'application/json'}
        print(f"AI client initialized with model: {model}")
    
    def query_ai(self, detections, prompt="Describe the scene based on these detections:"):
        """向DeepSeek-R1模型发送文本查询请求"""
        try:
            # 构建提示信息 - 只包含文本检测结果
            ...
            return response_data.get("response", "No response from AI model")
        
        except requests.exceptions.RequestException as e:
            print(f"Network error querying AI: {str(e)}", file=sys.stderr)
...
class AIWorker(QThread):
    ...
    def __init__(self, ai_client, detections, prompt):
        super().__init__()
    ...
    def run(self):
        try:
            if not self.detections:
                self.ai_response.emit("No detections to analyze.")
                return
            
            # 向AI发送查询请求 - 只传递检测结果和提示
            response = self.ai_client.query_ai(self.detections, self.prompt)
            self.ai_response.emit(response)
            
        except Exception as e:
            self.ai_error.emit(f"AI request error: {str(e)}")
...
class DetectionGUI(QMainWindow):
    def __init__(self, detector):
        super().__init__()
        self.detector = detector
        self.setWindowTitle("YOLOv8 Object Detection with DeepSeek-R1")
        ...
        self.result_list.setMinimumWidth(350)
        self.result_list.setMaximumHeight(250)
        detection_layout.addWidget(self.result_list)
        
        # AI分析面板
        ai_group = QGroupBox("DeepSeek-R1 Analysis")
        ai_layout = QVBoxLayout(ai_group)
        
        # AI输出区域
        self.ai_output = QTextEdit()
        self.ai_output.setReadOnly(True)
        self.ai_output.setPlaceholderText("DeepSeek-R1 analysis will appear here...")
        ...
        self.prompt_input.setPlaceholderText("Enter custom prompt for DeepSeek-R1...")
        self.prompt_input.setText("Describe the scene based on these detections:")
        prompt_layout.addWidget(self.prompt_input)
        
        # 控制按钮
        button_layout = QHBoxLayout()
        
        self.ai_button = QPushButton("Ask DeepSeek-R1")
        self.ai_button.clicked.connect(self.ask_ai)
        self.ai_button.setToolTip("Send current detections to DeepSeek-R1 for analysis")
        ...
        ai_layout.addLayout(prompt_layout)
        ...
        # 创建AI客户端 - 指定使用DeepSeek-R1模型
        self.ai_client = AIClient(model="qwen2:0.5b")
        self.ai_worker = None
    ...
    def ask_ai(self):
        """向DeepSeek-R1发送分析请求"""
        if not self.image_processor.last_detections:
            QMessageBox.warning(self, "AI Analysis", "No detections available for analysis.")
            return
        
        # 获取自定义提示
        custom_prompt = self.prompt_input.toPlainText().strip()
        if not custom_prompt:
            custom_prompt = "Describe the scene based on these detections:"
        
        # 在输出区域显示状态信息
        self.ai_output.clear()
        self.ai_output.append("Querying DeepSeek-R1 model...")
        self.ai_output.append("Please wait...")
        
        # 禁用按钮避免重复请求
        self.ai_button.setEnabled(False)
        self.ai_button.setText("Processing...")
        ...
        self.ai_worker = AIWorker(
            self.ai_client,
            self.image_processor.last_detections,
            custom_prompt
        )
        self.ai_worker.ai_response.connect(self.handle_ai_response)
        self.ai_worker.ai_error.connect(self.handle_ai_error)
        self.ai_worker.finished.connect(self.ai_request_finished)
        self.ai_worker.start()
    
    def handle_ai_response(self, response):
        """处理AI响应"""
        self.ai_output.clear()
        self.ai_output.append("DeepSeek-R1 Response:")
    ...
    def handle_ai_error(self, error):
        """处理AI错误"""
        self.ai_output.clear()
        self.ai_output.append("Error:")
        self.ai_output.append(error)
    
    def ai_request_finished(self):
        """AI请求完成时的清理工作"""
        self.ai_button.setEnabled(True)
        self.ai_button.setText("Ask DeepSeek-R1")
        self.status_bar.showMessage("AI analysis completed")
